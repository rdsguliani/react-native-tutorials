  ...
  return async (dispatch, getState) => {
    try {
      ...
      const response = await fetch(
        `https://rn-complete-guide-7ad70-default-rtdb.firebaseio.com/orders/${userId}.json`
      );
...
export const addOrder = (cartItems, totalAmount) => {
  return async (dispatch, getState) => {
    try {
      ...
      const userId = getState().auth.userId;
      const response = await fetch(
        `https://rn-complete-guide-7ad70-default-rtdb.firebaseio.com/orders/${userId}.json?auth=${
